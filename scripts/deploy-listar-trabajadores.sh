#!/bin/bash
# Script para desplegar listarTrabajadores usando AWS CLI directamente
# Esto evita el problema del template CloudFormation demasiado grande
...
echo -e "${GREEN}📦 Desplegando listarTrabajadores...${NC}"
...
mkdir -p handlers/usuarios shared utils

cp "$OLDPWD/handlers/usuarios/listarTrabajadores.js" handlers/usuarios/
...
if aws lambda get-function --function-name "$FUNCTION_NAME" --region "$REGION" &>/dev/null; then
  ...
  aws lambda update-function-configuration \
    --function-name "$FUNCTION_NAME" \
    --environment "Variables={TENANT_ID=utec,STAGE=$STAGE,REGION=$REGION,TABLA_USUARIOS=TablaUsuarios-$STAGE,TOKEN_SECRET=alerta-utec-123}" \
    --timeout "$TIMEOUT" \
    --memory-size "$MEMORY" \
    --region "$REGION" > /dev/null
else
...
# Crear recurso /usuarios si no existe
USUARIOS_RESOURCE_ID=$(aws apigateway get-resources \
  --rest-api-id "$API_ID" \
  --region "$REGION" \
  --query "items[?path=='/usuarios'].id" \
  --output text 2>/dev/null || echo "")

if [ -z "$USUARIOS_RESOURCE_ID" ]; then
  echo "📝 Creando recurso /usuarios..."
  USUARIOS_RESOURCE_ID=$(aws apigateway create-resource \
    --rest-api-id "$API_ID" \
    ...
    --query 'id' \
    --output text)
  echo -e "${GREEN}✅ Recurso /usuarios creado${NC}"
else
  echo "✅ Recurso /usuarios ya existe"
fi

# Crear recurso /trabajadores
TRABAJADORES_RESOURCE_ID=$(aws apigateway get-resources \
  --rest-api-id "$API_ID" \
  --region "$REGION" \
  --query "items[?path=='/usuarios/trabajadores'].id" \
  --output text 2>/dev/null || echo "")

if [ -z "$TRABAJADORES_RESOURCE_ID" ]; then
  echo "📝 Creando recurso /usuarios/trabajadores..."
  TRABAJADORES_RESOURCE_ID=$(aws apigateway create-resource \
    --rest-api-id "$API_ID" \
    --parent-id "$USUARIOS_RESOURCE_ID" \
    --path-part "trabajadores" \
    --region "$REGION" \
    --query 'id' \
    --output text)
  echo -e "${GREEN}✅ Recurso /usuarios/trabajadores creado${NC}"
else
  echo "✅ Recurso /usuarios/trabajadores ya existe"
fi

# Crear método GET
echo "📝 Configurando método GET..."
aws apigateway put-method \
  --rest-api-id "$API_ID" \
  --resource-id "$TRABAJADORES_RESOURCE_ID" \
  --http-method GET \
  --authorization-type NONE \
  --region "$REGION" > /dev/null 2>&1 || echo -e "${YELLOW}⚠️ Método GET ya existe${NC}"
...
aws apigateway put-integration-response \
  ...
  --http-method OPTIONS \
  --status-code 200 \
  --response-parameters '{"method.response.header.Access-Control-Allow-Origin":"'\''*'\''","method.response.header.Access-Control-Allow-Headers":"'\''Content-Type,X-Amz-Date,Authorization,X-Api-Key,X-Amz-Security-Token,X-Amz-User-Agent,X-Amzn-Trace-Id'\''","method.response.header.Access-Control-Allow-Methods":"'\''OPTIONS,GET'\''","method.response.header.Access-Control-Allow-Credentials":"'\''false'\''"}' \
  --region "$REGION" > /dev/null

# Configurar CORS en la respuesta del método GET también
echo "🌐 Configurando CORS en respuesta GET..."
aws apigateway put-method-response \
  --rest-api-id "$API_ID" \
  --resource-id "$TRABAJADORES_RESOURCE_ID" \
  --http-method GET \
...
# Crear integración response para GET con CORS
aws apigateway put-integration-response \
  --rest-api-id "$API_ID" \
  --resource-id "$TRABAJADORES_RESOURCE_ID" \
  --http-method GET \
...
echo ""
echo "📍 Endpoint: https://$API_ID.execute-api.$REGION.amazonaws.com/$STAGE/usuarios/trabajadores"
echo "🔗 Método: GET"
