#!/bin/bash
# Script para desplegar loginUsuario usando AWS CLI directamente
# Incluye CORS completo para compatibilidad con AWS Amplify

set -e

# Configuración
FUNCTION_NAME="hackathon-utec-incidentes-dev-loginUsuario"
REGION="us-east-1"
...
echo -e "${GREEN}📦 Desplegando loginUsuario...${NC}"
...
cp "$OLDPWD/handlers/auth/loginUsuario.js" handlers/auth/
...
if [ -z "$LOGIN_RESOURCE_ID" ]; then
  echo "📝 Creando recurso /auth/login..."
  LOGIN_RESOURCE_ID=$(aws apigateway create-resource \
    ...
    --query 'id' \
    --output text)
  echo -e "${GREEN}✅ Recurso /auth/login creado${NC}"
else
  echo "✅ Recurso /auth/login ya existe"
fi
...
echo ""
echo "📍 Endpoint: https://$API_ID.execute-api.$REGION.amazonaws.com/$STAGE/auth/login"
echo "🔗 Método: POST"
