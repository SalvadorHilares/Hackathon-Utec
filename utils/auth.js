...
// Para WebSocket: el token puede venir en query string ($connect) o en el body
function verifyJwtFromWebSocket(event, secretOverride) {
  // Intentar obtener token de query string (para $connect)
  const queryParams = event.queryStringParameters || {};
  let token = queryParams.token || queryParams.authorization;
  
  // Si no está en query string, intentar del body (para mensajes)
  if (!token) {
    try {
  ...
  // También intentar desde headers (por si acaso)
  if (!token) {
    const headers = event.headers || {};
    const authHeader = headers.authorization || headers.Authorization;
    if (authHeader && authHeader.startsWith('Bearer ')) {
