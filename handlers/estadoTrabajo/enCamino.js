...
async function handler(event) {
  try {
    ...
    const fecha_en_camino = getTimestamp();
    estadoTrabajo.estado_trabajo = 'en_camino';
    estadoTrabajo.fecha_en_camino = fecha_en_camino;
    ...
    const historial = {
      reporte_id,
      timestamp_accion: fecha_en_camino,
      accion: 'trabajador_en_camino',
      usuario_id: trabajador_id,
      rol: 'trabajador',
      entidad_afectada: 'trabajo',
      detalles_antes: { estado_trabajo: 'asignado' },
      detalles_despues: { estado_trabajo: 'en_camino', fecha_en_camino },
      notas: body.comentarios || 'Trabajador en camino',
      ip_address: '',
      user_agent: ''
    };
    ...
    console.error('Error al actualizar estado en camino:', error);
    return {
      statusCode: 500,
      body: JSON.stringify({
